...
// 创建独立连接（每次调用返回新连接，不写入共享字段）
func (r *RabbitMQ) mqConnect() (*amqp.Connection, error) {
...
// consumeWithReconnect 带重连机制的消费者，每个协程拥有独立连接和channel
func (r *RabbitMQ) consumeWithReconnect(b *BusinessConfig, handle Handle) {
	for {
		...
		conn, err := r.mqConnect()
		if err != nil {
			log.Printf("消费者[%s]连接失败，3秒后重试 err=%v", b.Name, err)
		...
		case <-r.done:
			return
		default:
			log.Printf("消费者[%s]连接断开，3秒后重连", b.Name)
			r.waitOrDone(3 * time.Second)
		}
	}
}

// receiver 消费消息，连接和channel作为参数传入（不使用共享字段）
